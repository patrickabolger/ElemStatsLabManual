...
Presumably, these data-collection software programs do this so that people don't have to figure out which program to use to open these files. This is useful for novices, but  a little annoying for experts.
```
...
```{block, type="rmdnote"}
**NOTE**: You might be wondering why we need two equals signs (==) in the computed variable, and not just a single equals sign (=). Recall that jamovi is based on R. In R, two equals signs are used for evaluating particular values. In this case, it can be interpreted as "where it's equal to." In contrast, a single equals sign (=) is used for various other, common operations, including the creation of variables, the assignment of arguments to functions, etc.
