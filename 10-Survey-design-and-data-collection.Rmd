...
##### Additional ~~requirement~~ **issue**: reverse-scoring {#AdditionalRequirementReverseScoring}

```{block2, type="rmdnote", echo=T}
This is no longer a requirement in this course. It does come up, however, if students intentionally or inadvertently create a reverse-scored item. See section \@ref(PreparingDataReverseScoring) for how to deal with this situation.
...
You could just have participants input a number like they do for `Age` (see section \@ref(GenderAndAge) below). This would be carried out through the *Short answer* question-type option in Google Forms, where you also included response validation by clicking the $\vdots$ symbol at the lower-right and checking *Response validation*. From there, you can choose from several parameters to "force" your participant to type in the type of number you need (e.g., decimal, integer, etc.). See Figure \@ref(fig:UsingOtherContinuousVariablesResponseValidation) below:
